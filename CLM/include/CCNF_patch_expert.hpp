...
public:
	...
	void Response(const cv::Mat_<float> &area_of_interest, cv::Mat &response);

	void ResponseOpenBlas(const cv::Mat &area_of_interest, cv::Mat &response, cv::Mat_<float> &im2col_prealloc);
