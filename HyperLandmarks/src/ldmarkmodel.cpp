...
ldmarkmodel::ldmarkmodel(std::string faceModel){
    ...
    current_shapes = std::vector<cv::Mat>(MAX_FACE_NUM);
...
std::vector<cv::Point> ldmarkmodel::track(const cv::Mat& src){
    cv::Mat grayImage;
	...
	for (int i = 0; i < MAX_FACE_NUM; i++)
	{
		...
		if (!current_shapes[i].empty()){
			faceBox[i] = get_enclosing_bbox(current_shapes[i]);
		}
		else{
	...
	for (int k = 0; k < MAX_FACE_NUM; k++){
		if (faceBox[k].area()<10000 || (double)faceBox[k].width / (double)faceBox[k].height > 1.08)
		{
			current_shapes[k] = cv::Mat();
	...
	for (int k = 0; k < MAX_FACE_NUM; k++){
        if (faceBox[k] != cv::Rect(0, 0, 0, 0)){
			current_shapes[k] = align_mean(meanShape, faceBox[k], 1.0, 1.0, 0.0, 0.0);
			int numLandmarks = current_shapes[k].cols / 2;
			for (int i = 0; i < LinearRegressors.size(); i++){
				cv::Mat Descriptor = CalculateHogDescriptor(grayImage, current_shapes[k], LandmarkIndexs.at(i), eyes_index, HoGParams.at(i));
				cv::Mat update_step = LinearRegressors.at(i).predict(Descriptor);
				if (isNormal){
					float lx = (current_shapes[k].at<float>(eyes_index.at(0)) + current_shapes[k].at<float>(eyes_index.at(1)))*0.5;
					float ly = (current_shapes[k].at<float>(eyes_index.at(0) + numLandmarks) + current_shapes[k].at<float>(eyes_index.at(1) + numLandmarks))*0.5;
					float rx = (current_shapes[k].at<float>(eyes_index.at(2)) + current_shapes[k].at<float>(eyes_index.at(3)))*0.5;
					float ry = (current_shapes[k].at<float>(eyes_index.at(2) + numLandmarks) + current_shapes[k].at<float>(eyes_index.at(3) + numLandmarks))*0.5;
					float distance = sqrt((rx - lx)*(rx - lx) + (ry - ly)*(ry - ly));
					update_step = update_step*distance;
				}
				current_shapes[k] = current_shapes[k] + update_step;
			}
		}
	}
    
    // return the first face
    int n = current_shapes[0].cols / 2;
    lmks.clear();
    for(int i = 0; i < n; i++)
        lmks.push_back(cv::Point(current_shapes[0].at<float>(i), current_shapes[0].at<float>(i+n)));
